...
for _, fname in ipairs(mouseFunctions) do
    test(fname, {}, function()
        local f = rawget(_G, fname) or rawget(_G, fname:lower())
...
test("fireclickdetector", {}, function()
    ...
    assert(detector:IsDescendantOf(workspace), "ClickDetector not parented correctly")
...
test("firetouchinterest", {}, function()
	...
	task.wait(1)
	firetouchinterest(char.PrimaryPart, part, 0)
	task.wait(0.2)
	firetouchinterest(char.PrimaryPart, part, 1)
	part:Destroy()
end)
...
test("Drawing", {}, function()
	assert(Drawing ~= nil, "Drawing is not defined")
	assert(type(Drawing) == "table", "Drawing should be a table")
	return "Drawing module is available"
end)
...
test("WebSocket", {}, function()
    if typeof(WebSocket) == "table" or typeof(WebSocket) == "userdata" then end
end)
